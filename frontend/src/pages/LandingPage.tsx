  ...
  return (
    <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-1/2 lg:block">
      <svg viewBox="0 0 120 110" preserveAspectRatio="xMidYMid meet" className="h-full w-full" aria-hidden="true">
        ...
        {edges.map(([a,b],i) => (
          <line key={`s${i}`} x1={nodes[a].cx} y1={nodes[a].cy} x2={nodes[b].cx} y2={nodes[b].cy}
            stroke="#ffffff" strokeOpacity="0.07" strokeWidth="0.6" />
        ))}
        {edges.map(([a,b],i) => (
          <line key={`e${i}`} x1={nodes[a].cx} y1={nodes[a].cy} x2={nodes[b].cx} y2={nodes[b].cy}
            stroke="#ffffff" strokeWidth="0.8" className="n-edge"
            style={{ animationDelay: `${(i * 0.29) % 3.6}s` }} />
        ))}
        {nodes.map((n,i) => (
          <circle key={i} cx={n.cx} cy={n.cy} r={n.r} fill="url(#nglow)" className="n-node"
            style={{ '--rb': `${n.r}px`, animationDelay: `${n.delay}s` } as React.CSSProperties} />
        ))}
      </svg>
    </div>
  );
