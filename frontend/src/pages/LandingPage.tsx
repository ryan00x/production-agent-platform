  ...
  return (
    <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-1/2 lg:block">
      <svg viewBox="0 0 120 110" preserveAspectRatio="xMidYMid meet" className="h-full w-full" aria-hidden="true">
        ...
        {dots.map((d, i) => (
          <circle key={i} cx={d.cx} cy={d.cy} r={d.r} fill="#06b6d4" fillOpacity={d.op} />
        ))}
...
function CircuitTraces() {
  ...
  return (
    <div className="pointer-events-none absolute inset-y-0 left-0 hidden w-[18%] lg:block opacity-[0.80]">
      <svg viewBox="0 0 80 110" preserveAspectRatio="xMaxYMid meet" className="h-full w-full" aria-hidden="true">
        ...
        {traces.map((t, ti) => {
          ...
          return (
            <g key={ti}>
              <path d={pathD} fill="none" stroke="#06b6d4" strokeOpacity="0.20" strokeWidth="0.7" />
              <circle r="1.6" fill="#06b6d4" fillOpacity="0.9" className="sig"
                style={{
                  offsetPath: `path('${mainPath}')`,
                  animationDelay: `${t.delay}s`,
                } as React.CSSProperties} />
              <circle cx={4}    cy={t.y} r="1.6" fill="none" stroke="#06b6d4" strokeOpacity="0.30" strokeWidth="0.6" />
              <circle cx={t.len} cy={t.y} r="1.6" fill="none" stroke="#06b6d4" strokeOpacity="0.30" strokeWidth="0.6" />
            </g>
          );
        })}
      </svg>
...
function BinaryRain() {
  ...
  return (
    <div className="pointer-events-none absolute inset-y-0 right-0 hidden w-[22%] lg:block opacity-[0.75]">
      <svg viewBox="0 0 110 110" preserveAspectRatio="xMinYMid meet" className="h-full w-full" aria-hidden="true">
        <defs>
          <style>{`
            ...
            .bit { font-family: monospace; font-size: 6px; fill: #06b6d4; animation: rain-fall 3.2s ease-in-out infinite; }
          `}</style>
        </defs>
        {cols.map((col, ci) =>
          col.digits.map((d, di) => (
...
function OrbitRings() {
  ...
  return (
    <div className="pointer-events-none absolute inset-y-0 left-0 hidden w-[18%] lg:block opacity-[0.75]">
      <svg viewBox="0 0 100 110" preserveAspectRatio="xMaxYMid meet" className="h-full w-full" aria-hidden="true">
        ...
        {rings.map((r, i) => (
          <ellipse key={`r${i}`} cx={r.cx} cy={r.cy} rx={r.rx} ry={r.ry}
            fill="none" stroke="#06b6d4" strokeOpacity="0.22" strokeWidth="0.7" />
        ))}
        <circle cx={50} cy={55} r={3} fill="#06b6d4" fillOpacity="0.25" />
        <circle cx={50} cy={55} r={1.2} fill="#06b6d4" fillOpacity="0.9" />
        {rings.map((r, i) => (
          <g key={`g${i}`} style={{ transformOrigin: `${r.cx}px ${r.cy}px`, animation: `orbit-spin ${r.speed} linear infinite` }}>
            ...
            >
              <animate attributeName="r" values="2;3;2" dur="2s" repeatCount="indefinite" begin={`${r.dotDelay}s`} />
              <animate attributeName="fill-opacity" values="0.6;1;0.6" dur="2s" repeatCount="indefinite" begin={`${r.dotDelay}s`} />
            </circle>
          </g>
        ))}
      </svg>
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      <svg viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" className="h-full w-full" aria-hidden="true">
        ...
        {pts.map(([x, y], i) => {
          const baseOp = 0.08 + (i % 5) * 0.04;
          const dur = 2.5 + (i % 7) * 0.6;
          return (
            <circle key={i} cx={x} cy={y} r={0.9 + (i % 3) * 0.5}
              fill="#06b6d4"
              className="star"
...
function RadarWheel() {
  ...
  return (
    <div className="pointer-events-none absolute -right-6 top-1/2 hidden w-[300px] -translate-y-1/2 xl:block opacity-[0.75]">
      <svg viewBox="0 0 100 100" className="h-full w-full" aria-hidden="true">
        ...
        <circle cx="50" cy="50" r="46" fill="none" stroke="#06b6d4" strokeOpacity="0.22" strokeWidth="0.5" />
        <circle cx="50" cy="50" r="34" fill="none" stroke="#06b6d4" strokeOpacity="0.15" strokeWidth="0.3" />
        ...
        <g className="wheel-rim">
          {labels.map((label, i) => {
            ...
            return (
              <text key={label} x={x} y={y} fontSize="2.6" fill="#06b6d4" fillOpacity="0.55"
...
export default function LandingPage() {
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#050505] text-white">
      ...
      {/* Nav — logo instead of plain text */}
        ...
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
          <Link to="/" className="flex items-center gap-2.5">
            <img
              src="/map-logo.png"
              alt="MAP"
              className="h-8 w-8 rounded-lg object-contain drop-shadow-[0_0_8px_rgba(6,182,212,0.5)]"
            />
      ...
      <section className="relative flex h-[88vh] min-h-[640px] items-center justify-center overflow-hidden border-b border-[#1E1E1E]">
        ...
        <div className="pointer-events-none relative z-10 px-6 text-center">
          {/* Logo mark above hero headline */}
          <div className="mb-6 flex justify-center">
            <img
              src="/map-logo.png"
              alt="MAP"
              className="h-16 w-16 rounded-2xl object-contain drop-shadow-[0_0_20px_rgba(6,182,212,0.6)]"
            />
          </div>
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-[#1E1E1E] bg-white/[0.04] px-3.5 py-1.5">
      ...
      {/* ── Footer CTA — particle field + logo ── */}
        ...
        <div className="relative z-10">
          <div className="mb-5 flex justify-center">
            <img
              src="/map-logo.png"
              alt="MAP"
              className="h-12 w-12 rounded-xl object-contain drop-shadow-[0_0_16px_rgba(6,182,212,0.5)]"
            />
          </div>
