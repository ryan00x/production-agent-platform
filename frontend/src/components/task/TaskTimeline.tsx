  ...
  return (
    <div className="glass-card p-6 border-slate-700/30">
      <div className="flex items-center gap-2 mb-8">
        ...
        <h3 className="font-bold text-lg text-white">Execution Timeline</h3>
      </div>

      <div className="flex flex-wrap gap-4 relative">
        {/* Progress Line - visible only on large screens when flexed horizontally */}
        <div className="hidden md:block absolute top-[18px] left-0 right-0 h-[1px] bg-white/10 -z-10" />
          ...
          return (
            <div key={step.id} className="flex-1 min-w-[200px] group">
              <div className="flex flex-col items-center md:items-start">
                {/* Status Dot */}
                <div className={`w-9 h-9 rounded-full flex items-center justify-center border-4 border-[#020617] relative z-10 transition-transform group-hover:scale-110 ${statusColor} text-white`}>
                ...
                <div className="mt-4 space-y-2 text-center md:text-left w-full">
                  <div className="flex flex-col">
                    <span className="text-xs font-bold text-white uppercase tracking-wider truncate" title={step.agent_name}>
                      {step.agent_name}
                    </span>
                  ...
                  <div className="p-3 rounded-xl bg-white/5 border border-white/10 space-y-1.5 transition-colors group-hover:bg-white/[0.08]">
        ...
        {steps.length === 0 && (
          <div className="w-full text-center py-10">
            <p className="text-slate-500 text-sm animate-pulse">Waiting for step timing data...</p>
          </div>
        )}
      </div>
    </div>
